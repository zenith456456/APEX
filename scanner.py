"""APEX-EDS v4.0 | scanner.py — 24x7 scan with diagnostics."""
import asyncio
import logging
import time
...
import config
from apex_engine import APEXEngine, log_diagnostic_summary
...
class Scanner:
    ...
    async def start(self):
        self._running = True
        logger.info("Scanner: 24x7 loop starting")
    ...
    async def _scan_all(self):
        ...
        for sym in symbols:
            ...
            if not dec.allow:
                logger.debug(f"BLOCKED [{sym}]: {dec.reason}")
                continue

            self._memory.record(result, prev=dec.prev)
            tn   = self._stats.record_signal(
                result.symbol, result.direction.value,
                result.entry_price, result.rr_ratio, result.score.total,
            )
            snap = self._stats.snapshot(tn)

            logger.info(
                f"★ SIGNAL #{tn} {result.symbol} {result.direction.value} "
                f"Score={result.score.total:.1f} RR={result.rr_ratio:.2f} "
                f"VPIN={result.vpin:.3f} CVD={result.cvd:+.3f} | "
                f"AllWR={snap['win_rate']:.1f}% "
                f"DayWR={snap['daily_win_rate']:.1f}%"
            )
        ...
        # Print diagnostic summary every 5 cycles
                    ...
                    if sd and sd.last_price > 0:
                        ...
                        if mem and old != mem.state:
                            if mem.state == TradeState.ALL_TP_HIT:
                                pnl = abs(mem.tp3 - mem.entry) / max(
                                    abs(mem.entry - mem.stop_loss), 0.0001
                                )
    ...
    async def _cleanup_loop(self):
        while self._running:
            ...
            st = self._stats
            logger.info(
                f"Stats: {st.total_trades} trades | "
                f"AllWR={st.win_rate:.1f}% "
                f"DayWR={st.daily_stats.win_rate:.1f}% "
                f"MonWR={st.monthly_stats.win_rate:.1f}% "
                f"PNL={st.total_pnl_r:+.2f}R"
            )
